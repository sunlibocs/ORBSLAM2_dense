...
std::string rgb_path = "/home/slbs/test_data/images/";
std::string depth_path = "/home/slbs/test_data/depthmaps/";
std::string seq_path = "/home/slbs/test_data/traj.txt";
...
bool rmd::test::Dataset::readDataSequence(size_t start, size_t end)
{
  ...
  //std::string path = "/home/slbs/Desktop/code/rpg_open_remode/test_data/first_200_frames_traj_over_table_input_sequence.txt";

  //std::ifstream sequence_file_str(sequence_file_path.string());
  std::ifstream sequence_file_str(seq_path);
...
bool rmd::test::Dataset::readImage(cv::Mat &img, const char *file_name) const
{
  ...
  std::string img_file_path = rgb_path + std::string(file_name);
  //img = cv::imread(img_file_path.string(), CV_LOAD_IMAGE_GRAYSCALE);
  img = cv::imread(img_file_path, CV_LOAD_IMAGE_GRAYSCALE);
  if(img.data == NULL)
    return false;
  else
    return true;
    ...
    const size_t &height) const
{
  ...
  std::string depthmap_file_path = depth_path + entry.getDepthmapFileName();
  std::ifstream depthmap_file_str(depthmap_file_path);
